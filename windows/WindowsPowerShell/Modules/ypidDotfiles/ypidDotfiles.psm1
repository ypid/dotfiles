...
Function Add-Path {
    ...
    if (Test-Path -PathType Container -Path $Path) {
        ...
        foreach ($env_single_path in $env_path_list) {
            try {
                [string] $sanitized_path = $env_single_path.TrimEnd("/", "\")
                $sanitized_path = [System.IO.DirectoryInfo]::new($env_single_path).FullName
                $env_paths.Add($sanitized_path, $true) | Out-Null
            } catch {}
        }
