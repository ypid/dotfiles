...
```Shell
datalad run "git ls-files -z | xargs --null -I '()' find './()' -maxdepth 0 -type f -print0 | xargs --null sed --in-place --regexp-extended 's/23/42/g;'"
...
git ls-files -z | xargs --null -I '()' find './()' -maxdepth 0 -type f -path "*prometheus*" -print0 | xargs --null perl -Wpi -e 'if (/(?<pre>.*instance=~")(?<instance>[^"]+)(?<post>".*)/) { my $pre = $+{pre}; my $post = $+{post}; my $esc_seq = "\\" x 2; (my $escaped = $+{instance}) =~ s/(\.(:?[^*]|$))/$esc_seq$1/g; $_ = "$pre$escaped$post\n" }'
EOF
```

`-maxdepth 0` causes the command to not operate on files in git submodules. Remove the `-maxdepth 0` if files in git submodules should be operated on as well.
